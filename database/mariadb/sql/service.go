...
// Notes: each entity service keeps its own base Service instance.
type Service struct {
	db  Database
	rec Record
}
...
// A shorcut of sql.ErrNoRows.
var ErrNoRows = sql.ErrNoRows

// GetByID binds a single record from the database to the `dest`.
...
// Where accepts a column name column value to set
...
func (opt ListOptions) BuildQuery() (q string, args []interface{}) {
	...
	if opt.OrderByColumn != "" {
		q += fmt.Sprintf(" ORGER BY %s %s", opt.OrderByColumn, ParseOrder(opt.Order))
	}
...
// Default to `ASC`. Two possible outputs: `ASC` and `DESC`.
